...
const DoctorRegistraition = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (hasErrors) {
      ...
      try {
        const res = await publicApiInstance.post(`/user/doctor/registration/`, data);
        console.log(res.data)
        setLoading(false)
        Toast().fire({
          title: "Check you mail to activate!",
          icon:"success",
        })
      } catch (error) {
        if (error.response) {
        ...
        // toast("Admin needs to validate your license to work properly.", {
          // duration: 5000,
        // });
        navigate("/login");
      }
